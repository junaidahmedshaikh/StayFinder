...
const SearchBar = () => {
  ...
  const handleIconClick = () => {
    inputRef.current?.showPicker(); // Triggers native date picker
  };
      ...
      <Card className="glass shadow-2xl border-0 bg-white-90 backdrop-blur-xl rounded-3xl overflow-hidden">
        <form onSubmit={handleSubmit} className="p-3">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
            ...
            {/* Check-in Input */}
            <div className="relative z-10 group border-2 border-red-500 border-solid ">
              <Calendar
                onClick={handleIconClick}
                className="absolute z-50 left-4 top-[65%] transform -translate-y-1/2 text-Rausch w-5 h-5 group-hover:text-gray-500 transition-colors"
              />
              <input
                type="date"
                placeholder="Check in"
                onChange={(e) =>
                  setUserSearch({ ...userSearch, checkIn: e.target.value })
                }
                ref={inputRef}
                className="pl-12 pr-4 border-0 focus:ring-2 focus:ring-babu-50 h-16 text-base bg-white-70 rounded-2xl backdrop-blur-sm hover:bg-white-90 transition-all duration-300"
              />
            ...
            <div className="relative z-10 group border-2 border-red-500 border-solid ">
              ...
              <input
                ...
                ref={inputRef}
                onChange={(e) => {
                  setUserSearch({ ...userSearch, checkOut: e.target.value });
                }}
                className="pl-12 pr-4 border-0 focus:ring-2 focus:ring-babu-50 h-16 text-base bg-white-70 rounded-2xl backdrop-blur-sm hover:bg-white-90 transition-all duration-300"
              />
              <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-babu-0 to-babu-0 group-hover:from-babu-5 group-hover:to-beach-5 transition-all duration-300 pointer-events-none" />
